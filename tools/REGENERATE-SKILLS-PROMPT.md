...
`tools/generate-skillmd.sh` wraps each flat skill into a discoverable `SKILL.md`, using
curated descriptions from `tools/skill-descriptions.tsv`. When the maintainer **adds new
...
tools/generate-skillmd.sh --check

# 2. Add a line per missing skill to tools/skill-descriptions.tsv (TAB-separated):
...
tools/generate-skillmd.sh
```

## LLM path — paste this to Claude Code

> Run `tools/generate-skillmd.sh --check` to list any flat skill sources that are missing
> from `tools/skill-descriptions.tsv`. For each missing skill: read its source file
...
> every source is covered, run `tools/generate-skillmd.sh` and confirm the new
...
tools/install-global.sh /custom/path    # -> a custom personal skills dir
```
